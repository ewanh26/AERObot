...
module.exports = {
    ...
    async execute(message, args, DISCORD, client) {
        ...
        const gamerRole = message.guild.roles.cache.find(r => r.name === 'Gamers');
        const karenRole = message.guild.roles.cache.find(r => r.name === 'Karens');
        const gamerEmoji = '🎮';
        const karenEmoji = '👩';
        ...
        client.on('messageReactionAdd', async (reaction, user) => {
            ...
            if (reaction.message.channel.id == channel) {
                ...
                if (reaction.emoji.name == karenEmoji) {
                    await reaction.message.guild.members.cache.get(user.id).roles.add(karenRole);
                }
            } else return;
        });
        ...
        client.on('messageReactionRemove', async (reaction, user) => {
            ...
            if (reaction.message.channel.id == channel) {
                ...
                if (reaction.emoji.name === karenEmoji) {
                    await reaction.message.guild.members.cache.get(user.id).roles.remove(karenRole);
                }
            } else return;
        });
    }
